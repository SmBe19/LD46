...
func help():
	send_output("Create request forwarding rule.\nIf a request can not be handled locally (because there is not matching service or the service is currently overloaded) it will be forwarded to other servers according to the rules defined here. If no rule is defined, the request is returned to the local queue.\n")
